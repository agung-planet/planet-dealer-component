...
class LinearGradient constructor(
    ...
    private val additionalOffsetX: Float = 0f,
    private val additionalOffsetY: Float = 0f,
    ...
    private fun getGradientCoordinates(size: Size): Pair<Offset, Offset> {
        ...
        val start = size.center + Offset(-horizontalOffset, verticalOffset) + Offset(
            (size.center.x * additionalOffsetX),
            0f
        )

        val end = size.center + Offset(horizontalOffset, -verticalOffset) + Offset(
            0f,
            (size.center.y * additionalOffsetY)
        )
...
fun Brush.Companion.linearGradient(
    ...
    additionalOffsetX: Float = 0f,
    additionalOffsetY: Float = 0f,
    angleInDegrees: Float = 0f,
    useAsCssAngle: Boolean = false
